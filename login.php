...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $query_entraineur = "SELECT * FROM entraineur WHERE email = ?";
  ...
  if ($result_entraineur->num_rows > 0) {
    ...
    if (password_verify($password, $entraineur['password'])) {
      $_SESSION['user_id'] = $entraineur['id'];
    $_SESSION['user'] = $entraineur['nom'];
    $_SESSION['role'] = 'coach';
    header("Location: entraineur.php");
    exit();
  }
  // Check if the user is a player
  $query_joueur = "SELECT * FROM joueurs WHERE email = ?";
  $stmt = $conn->prepare($query_joueur);
  $stmt->bind_param("s", $email);
